...
class FetchColorTemperatureRequestTests: XCTestCase {
    ...
    func testRequest() async throws {
        ...
        self.session.register(
            stub: MockHTTPSession.Stub(
                path: "/api/v1/\(token)/state/ct",
                method: .get,
                statusCode: 200,
                data: FetchColorTemperatureRequest.mock()
            )
        )
        
        let request = FetchColorTemperatureRequest(
